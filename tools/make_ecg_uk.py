# -*- coding: utf-8 -*-
"""
Builds ecg-uk.html — the UK twin of /ekg.

Same structure, design, sections and scripts as tools/make_ekg.py (hero,
stats, sample pages, real tracings, comparison, contents, the 8-step method,
parts, 2 bonus books, offer box, who it is for, author, recap, guarantee, FAQ,
final CTA). <head>, CSS and page scripts come from usd.html.

Differences from /ekg: British English and UK practice throughout (ECG, not
EKG, in the copy; Resuscitation Council UK; adrenaline; IEC lead colours; labs
in SI units), GBP pricing, the UK bonus sample pages, and noindex until the
checkout exists.

  python tools/make_ecg_uk.py
"""
...
BASE = (r"C:\Users\lucag\Desktop\CLAUDE\Infoproductos\Proyectos (código de los libros)"
        r"\Farmacologia\English")
LIBRO = os.path.join(BASE, "EKG-UK")
if not os.path.exists(os.path.join(LIBRO, "struttura.py")):
    LIBRO = os.path.join(BASE, "EKG-US")
os.environ["EDIZIONE"] = "UK"
_spec = importlib.util.spec_from_file_location("struttura_ecg_uk", os.path.join(LIBRO, "struttura.py"))
...
PRICE = 19.99  # 21/09/2026: £19.99 (prima £19, £24)
CURRENCY = "GBP"
TOTAL = 91
VAL_EM = 19
VAL_LAB = 17
VAL_ECG = TOTAL - VAL_EM - VAL_LAB
...
# The UK edition of the book is being typeset while this page is built, so the
# headings coming out of struttura.py are normalised to British spelling here.
# If the book's own file is already British, every rule below is a no-op.
BRIT = [
    (r"\bEKG paper\b", "ECG paper"),
    (r"\bIschemia\b", "Ischaemia"),
    (r"\bischemia\b", "ischaemia"),
    (r"\bArtifacts\b", "Artefacts"),
    (r"\bHyperkalemia\b", "Hyperkalaemia"),
    (r"\bHypokalemia\b", "Hypokalaemia"),
]
...
DESCR = {
    "I": "The paper, the twelve leads and the IEC lead colours: the vocabulary you need to read everything else.",
    ...
    "IV": "From ectopics to the arrest rhythms, split into shockable and non-shockable, the Resuscitation Council UK way.",
    ...
    "VII": "Potassium, calcium and the drugs that prolong the QT, in mmol/L: the ECG that changes with the bloods.",
    "VIII": "Artefacts and reversed leads, when to escalate straight away, and six guided cases.",
}
...
def gbp(x):
    # £19.99 con i decimali, £91 senza
    return f"£{x:,.2f}" if round(x) != x else f"£{x:,.0f}"
...
def sommario():
    ...
    for i, (num, tit, _sot, capitoli) in enumerate(S.PARTI, 1):
        ...
        voci = "".join(f"<li>{uk(t)}</li>" for t, _k in capitoli)
        v.append(f"""
        <div class="subject-item" data-open="false" role="listitem">
          <button class="subject-summary" type="button" aria-expanded="false" aria-controls="subj-content-{i}">
            ...
            <span class="subject-title">{uk(tit)} <em style="font-style:normal;opacity:.6;font-weight:600">&middot; pp. {da}-{a}</em></span>
    ...
    fine = "".join(f"<li>{uk(t)} &mdash; p. {S.CHIUSURA_PG[t]}</li>" for t, _v in S.CHIUSURA)
...
FAQ = [
    ...
    ("Is this a one-off payment or a subscription?",
     f"One-off payment of {gbp(PRICE)}. No instalments, no recurring charges, no surprises. "
     "You pay once and the material is yours to keep."),
    ("Do I need to know anything before I start?",
     "No, you start from zero: Part I explains the paper, the leads, where the electrodes go and the "
     "IEC colours before any arrhythmia. If you already read ECGs, you get the 8-step method and the "
     "chapters on infarction and electrolytes."),
    ("Are the tracings real or drawn?",
     "Real. They come from PTB-XL, a public collection of 21,799 12-lead ECGs from 18,869 patients, "
     "reported by cardiologists and published on PhysioNet. A drawn tracing is always clean; the ones "
     "you meet on the ward are not, and those are the ones to train your eye on."),
    ("Does it replace a course or clinical judgement?",
     "No. It is study material to understand and recognise, not a clinical protocol, and it is not "
     "affiliated with or endorsed by the NHS or any trust. With a real patient you follow your trust's "
     "policies and escalate when in doubt: the book helps you get there prepared."),
    ("Can I print the material?",
     "Yes, for personal use and without limits. The PDFs are high resolution, made to print, "
     "annotate and take on to the ward or to placement."),
]
...
def schema():
    faq = [{"@type": "Question", "name": d, "acceptedAnswer": {"@type": "Answer", "text": re.sub(r"&[a-z]+;", "", r)}}
           for d, r in FAQ]
    dati = {"@context": "https://schema.org", "@graph": [
        {"@type": "Product", "name": f"{TITLE} — illustrated ECG guide, UK edition",
         "brand": {"@type": "Brand", "name": "Med Study Lab"},
         "description": (f"{N_PAG} illustrated pages on real ECG tracings: rhythm, arrhythmias, blocks, "
                         "infarction and electrolytes, for UK practice. With two bonus books."),
         "image": f"https://www.medicalstudylab.com/mockups/{MOCK}/hero.webp",
         "offers": {"@type": "Offer", "price": str(PRICE), "priceCurrency": CURRENCY,
                    ...
                    "hasMerchantReturnPolicy": {"@type": "MerchantReturnPolicy",
                                                ...
                                                "merchantReturnDays": 30, "applicableCountry": "GB"}}},
...
EXTRA_CSS = """
  <style>
    ...
    .uk-note{max-width:720px;margin:22px auto 0;padding:14px 18px;border-left:4px solid var(--primary);
      background:var(--primary-light);border-radius:10px;font-size:15.5px;line-height:1.65;
      color:var(--text);text-align:left}
    .uk-note strong{color:var(--primary-dark)}
  </style>
"""
...
def corpo(FOOTER):
  ...
  <section class="hero">
    <div class="hero-content">
      <div class="hero-mockup">
        <img src="./mockups/{MOCK}/hero.webp" alt="{TITLE}, with Emergency Medications and How to Read Lab Tests as bonus books"
      ...
      <a href="#offer" id="cta-hero" class="btn-cta">I WANT TO READ ECGs WITH CONFIDENCE</a>
    ...
    <div class="container">
      <div class="stats-grid">
        <div class="stat-item">
          ...
          <p>from 21,799 ECGs reported by cardiologists, not drawn</p>
        </div>
        <div class="stat-item">
          <div class="icon">{ICO_LIB}</div>
          <strong>{N_PAG} illustrated pages</strong>
          <p>{N_PARTI} parts and {N_CAP} chapters, from the paper to infarction</p>
        </div>
        <div class="stat-item">
          <div class="icon">{ICO_MONDO}</div>
          <strong>UK edition</strong>
          <p>Resuscitation Council UK, adrenaline, IEC lead colours, SI units</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section section-alt section-snug-top">
    <div class="container">
      ...
      <div class="solution-content">
        ...
        <p class="text-center">Then you get on to the ward and the strip in front of you <strong>looks nothing like the ones you studied</strong>.</p>
        <div class="solution-image">
          <img src="./mockups/{MOCK}/strip.webp" alt="A real lead II rhythm strip on ECG paper" width="1600" height="400" loading="lazy" decoding="async">
        </div>
        <p class="text-center">That's why every tracing in the book comes from <strong>PTB-XL</strong>: a public collection of <strong>21,799 12-lead ECGs from 18,869 patients</strong>, each one reported by cardiologists and published on PhysioNet.</p>
        <p class="text-center">They're printed on real ECG paper, at 25 mm per second and 10 mm per millivolt, just as they come out of the machine. The drawing around them tells you <strong>where to look</strong>; the tracing stays a real patient's.</p>
  ...
  <section class="subjects-section fade-in visible" aria-label="Table of contents">
    <div class="container">
      <div class="section-header">
        ...
        <p class="subjects-subtitle">From the ECG paper to infarction, electrolytes and cases at the bedside: {N_PAG} pages, all illustrated</p>
      </div>
  ...
  <section class="section section-alt">
    <div class="container">
      ...
      <div class="solution-content">
        ...
        <p class="text-center">It isn't &ldquo;just another PDF&rdquo;. It's a method in <strong>{N_PARTI} parts and {N_CAP} illustrated chapters</strong>, built on real tracings, so you <strong>understand what you see instead of memorising pictures</strong>.</p>
  ...
  <section class="section">
    <div class="container">
      ...
      <div style="margin: 20px auto 28px;">
        <img src="./mockups/{MOCK}/combo.webp" alt="{TITLE}: mockup with sample pages" width="1100" height="1620" loading="lazy" decoding="async" style="max-width: 480px; width: 100%; display: block; margin: 0 auto;">
      </div>
  ...
  <section class="value-section">
    <div class="container">
      ...
      <div class="bonus-grid">
        <div class="bonus-item-with-image">
          <div class="bonus-mockup">
            <img src="./mockups/{MOCK}/bonus-em.webp" alt="Emergency Medications, illustrated handbook" width="720" height="1000" loading="lazy" decoding="async">
          </div>
          <div class="bonus-text">
            <h3>BONUS 1: &ldquo;Emergency Medications&rdquo;, the illustrated handbook</h3>
            <p class="value">Standalone value: {gbp(VAL_EM)}</p>
            <p>82 pages on the drugs you reach for when a tracing becomes an emergency: <strong>adenosine, amiodarone, atropine and adrenaline</strong>, with the doses, strengths and dilutions worth knowing before the arrest call.</p>
            <p style="margin-top: 12px;">It completes Part IV of the book: you recognise the rhythm, and you already know what gets drawn up. The adult ALS algorithm follows Resuscitation Council UK guidance.</p>
            <div class="bonus-sfoglia"><figure><img src="./amostras/ecguk_em1.webp" alt="Antiarrhythmics and the adult ALS algorithm" width="{PAGES_W}" height="{PAGES_H}" loading="lazy" decoding="async"><figcaption>The adult ALS algorithm</figcaption></figure><figure><img src="./amostras/ecguk_em2.webp" alt="Drug card: adrenaline" width="{PAGES_W}" height="{PAGES_H}" loading="lazy" decoding="async"><figcaption>The adrenaline card</figcaption></figure></div>
          </div>
        </div>

        <div class="bonus-item-with-image">
          <div class="bonus-mockup">
            <img src="./mockups/{MOCK}/bonus-lab.webp" alt="How to Read Lab Tests" width="720" height="1000" loading="lazy" decoding="async">
          </div>
          <div class="bonus-text">
            <h3>BONUS 2: &ldquo;How to Read Lab Tests&rdquo;</h3>
            <p class="value">Standalone value: {gbp(VAL_LAB)}</p>
            <p>74 pages to read a set of bloods: <strong>potassium, adjusted calcium, troponin</strong> and the full blood count, in <strong>mmol/L</strong> and the rest of the SI units, with the critical values that get telephoned and the pitfalls that throw them off.</p>
            <p style="margin-top: 12px;">Read it next to Part VII: a peaked T wave and a potassium of 6.8 are the same emergency seen from two sides.</p>
            <div class="bonus-sfoglia"><figure><img src="./amostras/ecguk_lab1.webp" alt="Critical results, with values in SI units" width="{PAGES_W}" height="{PAGES_H}" loading="lazy" decoding="async"><figcaption>Critical results</figcaption></figure><figure><img src="./amostras/ecguk_lab2.webp" alt="Test card: potassium in mmol per litre" width="{PAGES_W}" height="{PAGES_H}" loading="lazy" decoding="async"><figcaption>The potassium card</figcaption></figure></div>
          </div>
        </div>
      </div>
      ...
      <div class="decision-box" id="offer">
        <img src="./mockups/{MOCK}/hero.webp" alt="{TITLE} with the two bonus books" class="db-mockup" width="1400" height="830" loading="lazy" decoding="async">
        <h3 class="db-title">{TITLE} + 2 Bonus Books</h3>
        <ul class="db-checklist">
          <li><span class="db-check">{SPUNTA}</span>{TITLE}: {N_PAG} illustrated pages<span class="db-price-val">{gbp(VAL_ECG)}</span></li>
          <li><span class="db-check">{SPUNTA}</span>Bonus 1: Emergency Medications<span class="db-price-val">{gbp(VAL_EM)}</span></li>
          <li><span class="db-check">{SPUNTA}</span>Bonus 2: How to Read Lab Tests<span class="db-price-val">{gbp(VAL_LAB)}</span></li>
        ...
        <div class="db-anchoring">
          <p class="db-old-price">Total value: {gbp(TOTAL)}</p>
          <p class="db-new-price-label">Today only:</p>
          <p class="db-new-price">{gbp(PRICE)}</p>
          <span class="db-discount-badge">YOU SAVE {gbp(TOTAL - PRICE)}</span>
        </div>
        <a href="{CHECKOUT}" data-checkout class="db-cta">YES, I WANT THE ECG KIT FOR {gbp(PRICE)}</a>
        <div class="db-trust">
        ...
        <p class="db-small-print">All 3 books arrive by email right after purchase. One-off payment, no instalments or hidden fees.</p>
      </div>
    </div>
  </section>
    ...
    <div class="container">
      ...
      <div class="paraquem-grid">
        <div class="paraquem-card">
          <img class="paraquem-foto" src="./img/ekg-persona-student.webp" alt="Student nurse" loading="lazy" width="900" height="672">
          <div class="paraquem-info">
            <h3>Student nurse or medical student</h3>
            <p>ECGs come up in exams and on placement, and the textbooks explain them as if you already knew. You need to start from the paper and get to infarction without skipping a step.</p>
          </div>
        </div>
        <div class="paraquem-card">
          <img class="paraquem-foto" src="./img/ekg-persona-tele.webp" alt="Staff nurse on the ward" loading="lazy" width="900" height="672">
          <div class="paraquem-info">
            <h3>Staff nurse, HCA or paramedic</h3>
            <p>The monitor alarms, the tracing changes, and you're the first to see it. You want to know in seconds whether it's an artefact or a reason to escalate straight away.</p>
          </div>
        </div>
        <div class="paraquem-card">
          <img class="paraquem-foto" src="./img/ekg-persona-bedside.webp" alt="Newly qualified nurse at the bedside" loading="lazy" width="900" height="672">
          <div class="paraquem-info">
            <h3>Newly qualified, or an F1 on nights</h3>
            <p>On nights or on a busy ward, you often read the ECG on your own. You need a safe method and a quick revision of blocks, ischaemia and electrolytes.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="container">
      ...
      <div class="author-content">
        ...
        <div class="author-bio">
          ...
          <p>Pharmacist and founder of Med Study Lab, the series of illustrated books for healthcare: pharmacology, clinical handbooks, laboratory tests and now ECGs.</p>
          <p>The method is always the same: <strong>visual clarity and one topic per page</strong>, because what the brain sees organised, it remembers.</p>
          <p>For ECGs there was one more problem: a drawn tracing teaches you to recognise a drawing. That's why <strong>every tracing in the book is real</strong>, taken from ECGs reported by cardiologists.</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section section-alt">
    <div class="container">
      ...
      <ul class="recap-list">
        <li>{TITLE}: {N_PAG} illustrated pages in {N_PARTI} parts, from the ECG paper to infarction and electrolytes, on real tracings</li>
        <li>Written for UK practice: Resuscitation Council UK guidance, adrenaline, IEC lead colours and SI units</li>
        <li>Bonus 1: &ldquo;Emergency Medications&rdquo;, illustrated handbook, 82 pages (value {gbp(VAL_EM)})</li>
        <li>Bonus 2: &ldquo;How to Read Lab Tests&rdquo;, 74 pages (value {gbp(VAL_LAB)})</li>
        <li>Delivery: all 3 books by email right after purchase</li>
        <li>30-day guarantee, no questions asked: try it risk-free</li>
        <li>High-resolution PDFs: print them, annotate them, take them on to the ward</li>
      </ul>
      <div class="price-recap">
        <p class="old-price">{gbp(TOTAL)}</p>
        <p class="new-price">{gbp(PRICE)}</p>
      </div>
  ...
  <section class="section">
    <div class="container">
      <div class="guarantee-box">
        ...
        <p>No questions. No small print. No hassle.</p>
  ...
  <section class="final-cta">
    <div class="container">
      ...
      <p>The ECG doesn't have to be the part you avoid.</p>
      <p>You don't have to memorise fifty patterns without understanding why they look that way.</p>
      ...
      <p>Eight steps, real tracings and two bonus books for the acute end of the ward.</p>
      <p><strong>Now it's your turn.</strong></p>
      <div class="cta-summary">
        <ul>
          <li>One-off payment: {gbp(PRICE)}</li>
          <li>Guarantee: 30 days, no questions asked</li>
          <li>Delivery: all 3 books right after purchase</li>
        </ul>
      </div>
      <a href="#offer" class="btn-cta">I WANT THE ECG KIT FOR {gbp(PRICE)}</a>
...
def testa(t):
    h = t[:t.index("</head>")]
    subs = [
        (r"<title>.*?</title>", f"<title>Reading ECGs Made Visual: the illustrated UK guide on real tracings | Med Study Lab</title>"),
        (r'<meta name="description" content=".*?">',
         f'<meta name="description" content="Rhythm, arrhythmias, blocks, infarction and electrolytes in {N_PAG} illustrated pages on real ECGs, for UK practice, plus 2 bonus books. {gbp(PRICE)}, 30-day guarantee.">'),
        (r'<link rel="canonical" href=".*?">', f'<link rel="canonical" href="{URL}">'),
        (r'<meta property="og:url" content=".*?">', f'<meta property="og:url" content="{URL}">'),
        (r'<meta property="og:title" content=".*?">', '<meta property="og:title" content="Reading ECGs Made Visual — UK edition | Med Study Lab">'),
        (r'<meta property="og:description" content=".*?">',
         '<meta property="og:description" content="Read an ECG without panicking: an 8-step method on real tracings, explained with pictures, for UK practice.">'),
        (r'<meta property="og:image" content=".*?">', f'<meta property="og:image" content="https://www.medicalstudylab.com/mockups/{MOCK}/og.jpg">'),
        (r'<meta property="og:image:alt" content=".*?">', '<meta property="og:image:alt" content="Reading ECGs Made Visual — Med Study Lab">'),
        (r'<meta name="twitter:title" content=".*?">', '<meta name="twitter:title" content="Reading ECGs Made Visual — UK edition | Med Study Lab">'),
        (r'<meta name="twitter:description" content=".*?">',
         f'<meta name="twitter:description" content="{N_PAG} illustrated pages on real ECGs + 2 bonus books.">'),
        (r'<meta name="twitter:image" content=".*?">', f'<meta name="twitter:image" content="https://www.medicalstudylab.com/mockups/{MOCK}/og.jpg">'),
        (r'<script type="application/ld\+json">.*?</script>', schema()),
        (r'<meta name="robots" content=".*?">',
         '<meta name="robots" content="index, follow">' if LIVE else '<meta name="robots" content="noindex, nofollow">'),
    ]
    for rx, new in subs:
        h, n = re.subn(rx, lambda _m: new, h, count=1, flags=re.S)
        if n != 1:
            raise SystemExit(f"not found in usd.html head: {rx}")
    h = h.replace('<html lang="en">', '<html lang="en-GB">')
...
def footer(t):
    ...
    attr = ('<p style="margin:10px 0 0;font-size:11px;opacity:.55;">ECG tracings from PTB-XL '
            '(Wagner et al., PhysioNet), released under the CC BY 4.0 licence, and the MIT-BIH '
            'databases (PhysioNet). Study material only: not affiliated with, nor endorsed by, the NHS '
            'or the Resuscitation Council UK.</p>')
    i = f.rindex("</div>")
    return f[:i] + attr + "\n    " + f[i:]
...
def coda(t):
    ...
    item = json.dumps({"item_id": SLUG, "item_name": TITLE, "price": PRICE, "currency": CURRENCY, "quantity": 1})
  ...
  <script>
    (function () {{
      ...
      dataLayer.push({{ event: 'view_item', ecommerce: {{ currency: CUR, value: {PRICE}, items: [ITEM] }} }});
      ...
      document.addEventListener('click', function (e) {{
        ...
        dataLayer.push({{ event: 'begin_checkout', cta_id: a.id || a.className || 'no-id',
                         ecommerce: {{ currency: CUR, value: {PRICE}, items: [ITEM] }} }});
...
def main():
    ...
    for bad in ("spToast", "Manchester", "Pharmacology Kit", "epinephrine", "AHA", "US edition", "$"):
        assert bad not in body, f"leftover in body: {bad}"
    ...
    print(f"{SLUG}.html: {len(html) // 1024} KB · {N_PAG} pages · {N_PARTI} parts · {N_CAP} chapters · "
          f"{gbp(PRICE)} (value {gbp(TOTAL)}) · robots={'index' if LIVE else 'noindex'}")
