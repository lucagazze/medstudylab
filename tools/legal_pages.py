# -*- coding: utf-8 -*-
"""
Builds the legal pages Stripe asks for on medicalstudylab.com: support, privacy, terms.
US English. refund-policy.html already exists and is not touched. Real data only: QUILLSTONE DIGITAL LLC (Wyoming),
1057 NW 136th Ave, Miami, FL 33182; brand Med Study Lab; payments by Stripe
through checkout.medicalstudylab.com; Meta Pixel and Microsoft Clarity on the site;
hosting on Vercel.
...
<p><strong>I don't recognize a charge on my statement.</strong><br>Purchases on this site appear as <strong>{DESCRIPTOR}</strong>. If you don't recognize a charge, please contact us before your bank and we'll look into it straight away.</p>
...
<h2>Who we are</h2>
<p>Med Study Lab is a brand of <strong>{CO}</strong>, {ADDR}.</p>
"""),
    "refund-policy": ("Refund Policy", f"""
<p>We want you to try Med Study Lab with no risk.</p>
...
<p>This policy explains what personal data we collect when you visit <strong>medicalstudylab.com</strong> or buy our digital products, why we use it and what rights you have, including under applicable US state privacy laws and, for customers in the UK or EU, the UK GDPR and EU GDPR.</p>
<h2>1. Who we are</h2>
<p><strong>{CO}</strong> (trading as Med Study Lab), {ADDR}, is the controller of your personal data. Contact: <a href="mailto:{EMAIL}">{EMAIL}</a>.</p>
...
<li><strong>Microsoft Clarity</strong> — anonymous website usage analytics.</li>
<li><strong>Vercel</strong> — website hosting.</li></ul>
<p>Some of these providers are in the United States. For customers outside the US, transfers are covered by appropriate safeguards such as Standard Contractual Clauses or the EU-US Data Privacy Framework and its UK Extension.</p>
...
<h2>7. Your rights</h2>
<p>You can ask to access, correct, delete or port your data, object to processing, or withdraw consent at any time by emailing <a href="mailto:{EMAIL}">{EMAIL}</a>. If you are in the UK or EU, you can also complain to your local data protection authority.</p>
<h2>8. Children</h2>
...
<p>These terms apply to your use of <strong>medicalstudylab.com</strong> and your purchase of Med Study Lab digital products, sold by <strong>{CO}</strong>, {ADDR} ("we", "us"). By buying, you agree to these terms.</p>
<h2>1. Products</h2>
<p>We sell illustrated digital study ebooks (PDF) for nursing and healthcare students. What each product includes is described on its offer page.</p>
...
<p>Our materials are study aids. They don't replace your nursing program's official materials, current drug references, your facility's protocols or professional clinical judgment, and results depend on each student.</p>
<h2>7. Liability</h2>
...
<head>
  ...
  <title>{title} — Med Study Lab</title>
  <meta name="description" content="{title} for Med Study Lab.">
  <link rel="canonical" href="https://www.medicalstudylab.com/{slug}">
...
<body>
  <header><div class="hc"><a href="/" class="logo">Med Study Lab</a><a href="/" class="back">← Back to Home</a></div></header>
  <main><div class="card">
    <h1>{title}</h1>
    <span class="upd">Last updated: {UPDATED}</span>
{body}
  </div></main>
  <footer><p>© 2026 Med Study Lab · {NAV}</p><p style="margin-top:8px;font-size:.75rem;opacity:.7">{CO} · {ADDR}</p></footer>
</body>
</html>
"""
...
def main():
    ...
    d = json.load(open(vj))
    have = {r["source"] for r in d.get("rewrites", [])}
    for slug in ("support", "privacy", "terms"):
        if "/" + slug not in have:
            d.setdefault("rewrites", []).append({"source": "/" + slug, "destination": f"/{slug}.html"})
    io.open(vj, "w", encoding="utf-8", newline="\n").write(json.dumps(d, indent=2) + "\n")
    # footer links on the landing pages
    s = ' style="color: rgba(255,255,255,0.7); text-decoration: underline; margin-left: 8px;"'
    old = f'<a href="/refund-policy"{s}>Refund Policy</a>'
    new = old + f'<a href="/privacy"{s}>Privacy</a><a href="/terms"{s}>Terms</a><a href="/support"{s}>Support</a>'
    for f in os.listdir(SITE):
        if not f.endswith(".html") or f in ("support.html", "privacy.html", "terms.html"):
            continue
        p = os.path.join(SITE, f)
        t = io.open(p, encoding="utf-8").read()
        if old in t and 'href="/privacy"' not in t:
